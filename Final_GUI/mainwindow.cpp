...
{   node_t * xx;
    ...
    for (int x_idx = 0; x_idx < size; x_idx++)
        for (int y_idx = 0; y_idx < size - 1; y_idx++){
            xx=(node_t *)parent->sub_array[y_idx];
            yy=(node_t *)parent->sub_array[y_idx+1];
           // printf("%s,    %s\n", xx->name, yy->name);
...
int MainWindow::dir_entry_count(const char* directory)
{
    ...
    while ((file_dirent = readdir(dir_handle)) != NULL)
    {
        ...
        strcat(path, "/");
        strcat(path, file_dirent->d_name);

        struct stat p_statbuf;
        lstat(path, &p_statbuf);
...
void MainWindow::deleteBack( node_t * parent){
    //if(strlen(parent->name)>0) free(parent->name);
    for(int i=0;i<parent->size_sub_dirs;i++){
        if(parent->sub_array[i]!=NULL) deleteBack((node_t *)parent->sub_array[i]);
    }
...
long long MainWindow::listdir(const char* name, node_t* parent, QTreeWidgetItem* Q_tree_ptr)
{
    ...
    struct dirent* entry;
    node_t* temp_node;
    if (!(dir = opendir(name)) || strcmp(name, "//proc") == 0 || strcmp(name, "//root") == 0 || strcmp(name, "//lost+found") == 0){ // check if the parent is a directory
        parent->size=0;
        parent->size_sub_dirs=0;
        return 0;
    }
    while ((entry = readdir(dir)) != NULL) // entry stores the sub-directory; each iteration for this loop, a new node is created with a sub_directory/file
    {
        ...
        if (entry->d_type == DT_DIR) //DT_DIR is the type of a directory, if d_type has this value, this node is a directory not a file
        {
            ...
            if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0)
                continue;
            if(strcmp(path, "//proc") == 0) printf("SSSSYMOBLIC LINK EH BA2AAAAA\n");
        ...
        counter++;}else free(temp_node);
        //if(temp_node->name!= NULL)printf("%s\n", temp_node->name);
        //else printf("NULL\n");
    }
    parent->size = size; // This size is set in the end after all iterations.
    if(parent->size_sub_dirs >0 ) bubble_sort(parent->size_sub_dirs, parent); // The chilren array is sorted in the end after all nodes are entered successfully
    closedir(dir);
    return size;
}
...
{
    float percentage;
    if(parent->sub_array == NULL || strcmp(parent->name, "proc") == 0) // if a given parent node has no children nodes
        return;
    node_t* temp_node;
    /*
    ...
    */
    for(int i = 0; i < parent->size_sub_dirs; i++)
    {
        ...
        if(strcmp(temp_node->name, "proc") == 0 || strcmp(temp_node->name, "root") == 0 || strcmp(temp_node->name, "lost+found") == 0) continue;
        ...
        delete((node_t *)parent->sub_array[i]);
    }
}
